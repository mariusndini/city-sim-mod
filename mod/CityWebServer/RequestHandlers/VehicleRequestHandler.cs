...
namespace CityWebServer.RequestHandlers
{
    ...
    public class VehicleRequestHandler : RequestHandlerBase
    {
        public VehicleRequestHandler(IWebServer server)
            : base(server, new Guid("2be6546a-d416-4939-8e08-1d0b739be835"), "Vehicle", "Marius", 100, "/Vehicle")
        {
        }
        ...
        public override IResponseFormatter Handle(HttpListenerRequest request)
        {
            ...
            if (request.Url.AbsolutePath.StartsWith("/Vehicle/List"))
            {
                ...
                {
                    if (vehicleManager.m_vehicles.m_buffer[i].m_flags == Vehicle.Flags.Stopped) { continue; }
            ...
            {
                if (vehicle.m_flags == Vehicle.Flags.Stopped) { continue; }
